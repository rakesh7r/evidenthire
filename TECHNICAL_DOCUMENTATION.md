...
```mermaid
erDiagram
    ORGANIZATION ||--|{ USER_ACCOUNT : has
    ORGANIZATION ||--|{ POSITION : posts
    ...
    POSITION ||--|{ APPLICATION : receives
    POSITION ||--|{ INTERVIEW : defines
    POSITION {
        uuid id PK
        text title
        jsonb requirements
    ...
    CANDIDATE ||--|{ INTERVIEW : participates
    CANDIDATE {
        uuid id PK
        text email
        text name
    ...
    INTERVIEW ||--|{ MEDIA_CHUNK : generates
    INTERVIEW ||--|{ INTERVIEW_REPORT : produces
    INTERVIEW ||--|{ SIGNAL_QUALITY : evaluated_by
    INTERVIEW {
        uuid id PK
        text status
    ...
    MEDIA_CHUNK ||--|{ TRANSCRIPT_SEGMENT : contains
    MEDIA_CHUNK {
        uuid id PK
        text s3_uri
        int start_offset_ms
