...
export default function CreateInterviewModal({
	...
	if (!canCreate) {
		return (
			<div className='fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200'>
				<div className='w-full max-w-md overflow-hidden rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl p-6'>
					<h3 className='text-lg font-semibold text-red-500'>Permission Denied</h3>
					<p className='mt-2 text-slate-500 dark:text-slate-400'>
						Only Administrators and Recruiters can create interviews.
					</p>
					<div className='mt-6 flex justify-end'>
						<button
							onClick={onClose}
							className='rounded-lg bg-slate-100 dark:bg-slate-800 px-4 py-2 text-sm font-semibold text-slate-900 dark:text-white hover:bg-slate-200 dark:hover:bg-slate-700'>
							Close
						</button>
					</div>
				</div>
	...
	return (
		<div className='fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200'>
			<div className='w-full max-w-lg overflow-hidden rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]'>
				...
				<form
						...
						<div className='grid grid-cols-1 gap-4'>
							<div className='space-y-2'>
								...
								<input
									...
									className='w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 py-2.5 px-3 text-sm text-slate-900 dark:text-white placeholder-slate-500 focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500'
							...
							<div className='space-y-2'>
								...
								<div className='relative'>
									...
									<input
										...
										className='w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 py-2.5 pl-10 pr-3 text-sm text-slate-900 dark:text-white placeholder-slate-500 focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500'
						...
						<div className='grid grid-cols-2 gap-4'>
							<div className='space-y-2'>
								...
								<input
									...
									className='w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 py-2.5 px-3 text-sm text-slate-900 dark:text-white placeholder-slate-500 focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500 dark:scheme-dark'
								/>
							</div>
							<div className='space-y-2'>
									...
									<input
										...
										className={`w-full rounded-lg border ${
											!isTimeValid && date && time
												...
												: 'border-slate-200 dark:border-slate-700 focus:border-orange-500 dark:focus:border-orange-500 focus:ring-orange-500'
										} bg-white dark:bg-slate-800 py-2.5 pl-10 pr-3 text-sm text-slate-900 dark:text-white placeholder-slate-500 focus:outline-none focus:ring-1 dark:scheme-dark`}
									/>
								</div>
							</div>
						</div>
						...
						<div className='space-y-3'>
								...
								<div className='flex flex-wrap gap-2 mb-2'>
									{selectedInterviewerIds.map((id) => {
										...
										return (
											<div
												key={id}
												className='inline-flex items-center gap-1.5 rounded-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-3 py-1 text-xs text-slate-700 dark:text-slate-200'>
												<div className='bg-orange-500/20 text-orange-600 dark:text-orange-500 rounded-full w-5 h-5 flex items-center justify-center text-[10px] font-bold'>
													{user?.name.charAt(0)}
												</div>
												{user?.name}
												<button
													...
													className='ml-1 text-slate-400 dark:text-slate-500 hover:text-red-500 focus:outline-none'>
													<X className='h-3 w-3' />
												</button>
											</div>
										);
							...
							<div
								...
								{isInterviewerDropdownOpen && (
									<div className='absolute z-10 mt-1 w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-xl max-h-60 overflow-auto'>
											...
											<div className='py-1'>
												{filteredUsers.map((user) => (
													<button
														...
														className='w-full flex items-center gap-3 px-4 py-2 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors text-left group'>
														<div className='flex h-8 w-8 items-center justify-center rounded-full bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300 text-xs font-semibold'>
															{user.name.charAt(0)}
														</div>
														<div>
					...
					<div className='shrink-0 border-t border-slate-200 dark:border-slate-800 p-6 flex justify-end gap-3 bg-slate-50 dark:bg-slate-900'>
						<button
							type='button'
							onClick={onClose}
							className='rounded-lg px-4 py-2.5 text-sm font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors'>
							Cancel
						</button>
						<button
